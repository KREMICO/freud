...
export default function Mapa() {
  const { pontos, progresso, setFaseAtual, fasesConcluidas } = useGame();
    ...
    <main className="min-h-screen w-full bg-white text-black flex flex-col items-center justify-between p-8">
      ...
      <section className="relative w-[650px] h-[420px] border-4 border-black rounded-[50%] flex items-center justify-center">
        ...
        {fases.map((fase, index) => {
          const bloqueada = fasesConcluidas.includes(fase.id);

          return (
            <Link
              key={fase.id}
              href={bloqueada ? "#" : `/questions/${fase.id}`}
              onClick={(e) => {
                if (bloqueada) {
                  e.preventDefault();
                  return;
              ...
              className={`absolute w-24 h-24 rounded-full border-2 border-black transition flex items-center justify-center
                ${
                  bloqueada
